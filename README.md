...
- Not complete yet: explicit-row disclosure, reusable Agent definitions and workflow Agent steps, richer charts and reports, wall-clock/OS notifications, approval/side-effect workflow nodes, resumable partial runs, remote MCP/OAuth, approved MCP prompt/tool/model/Agent/workflow use, RAG, Hub/RBAC/sync, signing, and updates.
...
npm run smoke:mcp           # prove zero-call discovery and one exact approved local resource read
