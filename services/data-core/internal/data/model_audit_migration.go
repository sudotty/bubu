...
const modelAuditAggregateMigrationSQL = `
ALTER TABLE model_disclosure_outcomes RENAME TO model_disclosure_outcomes_v8;
ALTER TABLE model_disclosure_events RENAME TO model_disclosure_events_v8;
DROP INDEX model_disclosure_events_started_idx;

CREATE TABLE model_disclosure_events (
    id TEXT PRIMARY KEY,
    purpose TEXT NOT NULL CHECK (purpose IN ('provider-connection-test', 'query-plan', 'group-query-plan', 'aggregate-explanation')),
...
SELECT id, purpose, target_kind, target_id, disclosure, provider_id, provider_kind,
       provider_name, model, endpoint_origin, dataset_count, column_count,
       synthetic_row_count, 0, relationship_count, payload_bytes,
       estimated_input_tokens, max_output_tokens, payload_sha256, contains_raw_rows, started_at
FROM model_disclosure_events_v8;
...
SELECT disclosure_id, status, input_tokens, output_tokens, total_tokens,
       output_bytes, error, finished_at
FROM model_disclosure_outcomes_v8;

DROP TABLE model_disclosure_outcomes_v8;
DROP TABLE model_disclosure_events_v8;
