...
func TestOpenUpgradesAVersionOneCatalog(t *testing.T) {
	...
	var applied int
	if err := service.database.QueryRow("SELECT COUNT(*) FROM schema_migrations WHERE version IN (2, 3, 4, 5, 6)").Scan(&applied); err != nil {
		t.Fatal(err)
	}
	if applied != 5 {
		t.Fatal("version 2, 3, 4, 5, and 6 migrations were not applied")
	}
}
