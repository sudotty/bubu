...
const explicitRowAuditMigrationSQL = `
ALTER TABLE model_disclosure_outcomes RENAME TO model_disclosure_outcomes_v24;
ALTER TABLE model_disclosure_events RENAME TO model_disclosure_events_v24;
DROP INDEX model_disclosure_events_started_idx;

INSERT INTO model_disclosure_purposes(purpose) VALUES ('explicit-row-explanation');
...
CREATE TABLE model_disclosure_events (
    ...
    disclosure TEXT NOT NULL CHECK (disclosure IN ('none', 'schema-only', 'schema-synthetic', 'aggregates', 'explicit-rows')),
    ...
    raw_row_count INTEGER NOT NULL DEFAULT 0 CHECK (raw_row_count BETWEEN 0 AND 20),
...
INSERT INTO model_disclosure_events(
    ...
    synthetic_row_count, aggregate_row_count, raw_row_count, relationship_count, payload_bytes,
...
SELECT id, purpose, target_kind, target_id, disclosure, provider_id, provider_kind,
       provider_name, model, endpoint_origin, dataset_count, column_count,
       synthetic_row_count, aggregate_row_count, 0, relationship_count, payload_bytes,
       estimated_input_tokens, max_output_tokens, payload_sha256, contains_raw_rows, started_at
FROM model_disclosure_events_v24;
...
FROM model_disclosure_outcomes_v24;

DROP TABLE model_disclosure_outcomes_v24;
DROP TABLE model_disclosure_events_v24;
