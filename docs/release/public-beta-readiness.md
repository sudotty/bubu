...
- Pull requests build and exercise unsigned macOS arm64/x64 and Windows x64 installers without release credentials. The signed workflow independently requires an exact GitHub-verified signed annotated version tag and declares the `release` environment; the remote environment, reviewer approval, and publisher credentials remain an external blocker rather than an implemented repository setting.
