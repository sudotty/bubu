...
- Packaging-relevant pull requests build and exercise unsigned macOS arm64/x64 and Windows x64 installers without release credentials. The signed workflow is dispatched from protected `main`, requires an exact on-main GitHub-verified signed annotated version tag and successful source checks, then enters the owner-approved `release` environment. Publisher credentials and real signing evidence remain external blockers.
