...
| 2 | Active | Atomic CSV/TSV/XLSX import, SQLite catalog, immutable replacement, drift blocking, type inference/quality/on-demand distributions, safe query compiler, groups/relationships, Excel-safe export, deletion, verified backup/restore, and end-to-end cancellation |
...
The Stage 2 slice is exercised through Go integration tests and `npm run smoke:data-core`. Interactive drift mapping, bounded group lookup joins, local quality/validation, local-only on-demand histograms/Top values, reusable relationships, Excel-safe export, permanent deletion/group repair, hash/schema/privacy-verified local backup/restore, and named cancellation propagated through Electron, authenticated RPC, Go contexts, and AI provider signals are implemented. Remaining Stage 2 work is reference-device performance evidence.
