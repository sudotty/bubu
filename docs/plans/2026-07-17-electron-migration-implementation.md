...
| 2 | Active | Atomic CSV/TSV/XLSX import, SQLite catalog, immutable replacement, drift blocking, type inference/quality/on-demand distributions, safe query compiler, groups/relationships, Excel-safe export, deletion, and verified backup/restore |
...
The Stage 2 slice is exercised through Go integration tests and `npm run smoke:data-core`. Interactive drift mapping, bounded group lookup joins, local quality/validation, local-only on-demand histograms/Top values, reusable relationships, Excel-safe export, permanent deletion/group repair, and hash/schema/privacy-verified local backup/restore are implemented. Remaining Stage 2 work is cancellation and reference-device performance evidence.
