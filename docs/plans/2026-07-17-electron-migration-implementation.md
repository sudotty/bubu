...
| 2 | Active | Atomic CSV/TSV/XLSX import, SQLite catalog, immutable replacement, drift blocking, type inference/profiles/preview, safe query compiler, local 2–8-member dataset groups, Excel-safe CSV export, and transactional permanent deletion/group repair |
...
The Stage 2 slice is exercised through Go integration tests and `npm run smoke:data-core`. Interactive drift mapping, bounded group lookup joins, local quality reports, persistent deterministic validation rules, reusable current-version-checked relationships, current-version Excel-safe CSV export, and transactional permanent deletion/group repair are implemented. Remaining Stage 2 work is richer distributions, backup/recovery, cancellation, and reference-device performance evidence.
