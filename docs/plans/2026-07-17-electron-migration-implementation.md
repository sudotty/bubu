...
| 3 | Active | Schema-only/synthetic disclosure, encrypted credentials, provider registry/selection/tests, strict single/group plans, local execution approval, fail-closed disclosure/usage ledger, exact one-use k>=5 aggregate explanations, and audited bounded aggregate Agent turns are implemented; streaming, workflow Agent authorization, and explicit rows remain |
| 4 | Active | Dataset/group chat surfaces persist typed append-only questions/plans/source-linked results/cell-cited explanations and Agent reports with tool/audit traces/errors, then restore local tables/charts; tags, multiple threads, richer charts, and reports remain |
| 5 | Active | Versioned query workflows provide current-version rebinding, idempotency, bounded retries/deadlines, cancellation, typed checkpoints, local audit, persistent 24-hour/7-day/data-version triggers, restart recovery, and atomic chat delivery; the interactive Agent core is bounded and reusable; local stdio MCP registration/launch consent/lifecycle discovery plus exact approved local resource reads, local-only prompt gets, and no-content audit are implemented, while tool execution, prompt-to-model/Agent/workflow use, remote MCP/OAuth, durable workflow Agent steps, wall-clock/OS reminders, approvals, partial-run resume, and RAG remain |
| 6–7 | Pending | Must not be represented as shipped; follow the exit criteria above |

The Stage 2 slice is exercised through Go integration tests, `npm run smoke:data-core`, and `npm run verify:performance`. Interactive drift mapping, bounded group lookup joins, local quality/validation, local-only on-demand histograms/Top values, reusable relationships, Excel-safe export, permanent deletion/group repair, hash/schema/privacy-verified local backup/restore, named cancellation propagated through Electron/authenticated RPC/Go contexts/AI provider signals, and measured large-file budgets are implemented. Stage 2 has no remaining planned item.
