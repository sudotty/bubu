...
| 2 | Active | Atomic CSV/TSV/XLSX import, SQLite catalog, immutable replacement, drift blocking, type inference/profiles/preview, safe query compiler, groups/relationships, Excel-safe CSV export, permanent deletion, and verified backup/restore |
...
The Stage 2 slice is exercised through Go integration tests and `npm run smoke:data-core`. Interactive drift mapping, bounded group lookup joins, local quality reports, persistent deterministic validation rules, reusable current-version-checked relationships, current-version Excel-safe CSV export, transactional permanent deletion/group repair, and hash/schema/privacy-verified local backup/restore are implemented. Remaining Stage 2 work is richer distributions, cancellation, and reference-device performance evidence.
