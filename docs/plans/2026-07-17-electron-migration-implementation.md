...
| 3 | Active | Schema-only/synthetic disclosure context, OS-encrypted write-only credentials, provider registry/selection, packaged settings UI, connection tests, and bounded provider adapters; no data chat yet |
| 4–7 | Pending | Must not be represented as shipped; follow the exit criteria above |
