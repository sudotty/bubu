...
- Data objects and business topics appear only where they are relevant. Within an object or topic, a thread list makes the current task explicit; users can create, rename, resume, and archive independent local conversations without mixing their evidence. Settings uses the full workspace width and must not retain an unrelated object list.
...
- A chart is shown only when a pure suitability rule can explain it without inventing aggregation or hiding high cardinality. The UI always states the recommendation reason and supplies the exact plotted points as a table. Professional report bundles remain bounded, offline, escaped, and free of hidden source rows; HTML, PDF, XLSX, CSV, and manifest facts come from one strict report model.
...
- MCP metadata and results are labeled local-only and untrusted. A fetched prompt can reach a model only through a new exact disclosure review. A model may propose one schema-valid tool call, but a second independent review is required before execution; results are never automatically sent to a model, chat, Agent, or workflow.
...
- Remote MCP save, OAuth PKCE, inspection and exact tool execution are separate steps. The UI shows authorization status but never receives tokens; remote prompt/resource operations remain unavailable rather than appearing as inactive promises.
...
| Settings | Data objects remained visible even though they had no settings role | Give settings a focused two-column rail/workspace layout | `docs/assets/product/03-settings.png` |
