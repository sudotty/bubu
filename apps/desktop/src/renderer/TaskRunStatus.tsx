...
export function TaskRunStatus({ state, startedAt, completedAt }: { readonly state: TaskState; readonly startedAt?: number | undefined; readonly completedAt?: number | undefined }) {
  ...
  return <section className={`task-run-status task-${state}`} aria-live="polite">
    ...
    {state !== "idle" && <p>{state === "planning" || state === "proposed" ? "远程模型只参与生成类型化计划；数据行不会随请求自动发送。" : "确定性查询由本地 Go 数据内核执行，结果作为当前任务的 Artifact 保存。"}</p>}
  </section>;
}
