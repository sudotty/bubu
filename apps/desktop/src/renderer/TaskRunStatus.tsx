...
export function TaskRunStatus({ state, startedAt, completedAt }: { readonly state: TaskLifecycleState; readonly startedAt?: number | undefined; readonly completedAt?: number | undefined }) {
  ...
  const label = state === "draft" ? "等待一个新问题" : state === "planning" ? "正在生成受限计划" : state === "awaiting-approval" ? "等待你批准本地执行" : state === "executing" ? "正在本地执行" : state === "completed" ? "本地任务已完成" : state === "cancelled" ? "任务已取消，可安全重试" : "任务需要处理";
  ...
  return <section className={`task-run-status task-${state}`} aria-live="polite">
    ...
    {state !== "draft" && <p>{state === "planning" || state === "awaiting-approval" ? "远程模型只参与生成类型化计划；数据行不会随请求自动发送。" : state === "cancelled" ? "取消只终止当前操作，不会删除此前保存的问题、计划或结果。" : "确定性查询在本地执行，结果保存在当前任务中。"}</p>}
  </section>;
}
