...
function errorMessage(error: unknown): string {
  return error instanceof Error ? error.message : "本地数据保护操作失败";
}
...
export function DataProtectionPanel({
  ...
  async function createBackup(): Promise<void> {
    ...
    setNotice(undefined);
    try {
      const result = await window.bubu.dataProtection.createBackup();
  ...
  async function restoreBackup(): Promise<void> {
    ...
    setNotice(undefined);
    try {
      const result = await window.bubu.dataProtection.restoreBackup();
