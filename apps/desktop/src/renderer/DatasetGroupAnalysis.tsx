...
export function DatasetGroupAnalysis({ group, threadId, onCreateThread, onOpenArtifact }: { readonly group: DatasetGroup; readonly threadId?: string | undefined; readonly onCreateThread: () => Promise<void>; readonly onOpenArtifact: () => void }) {
  ...
  return (
    <section className="analysis-panel group-analysis" aria-label={`与群组 ${group.name} 对话`}>
      ...
      <ChatAssistantMessage title="关联结果已准备好"><p>关联计划已在本地执行。完整数据、图表、计划与审计证据都在结果区。</p><ChatResultFile title={proposal?.plan.purpose ?? submittedQuestion ?? "群组查询结果"} result={result} /><button type="button" className="chat-artifact-link" onClick={onOpenArtifact}>打开结果区</button></ChatAssistantMessage>
      </>}
