...
for (const value of ["signed-installers: planned", "native-package-matrix-ci: implemented", "draft-github-release-pipeline: implemented", "release-checksums: implemented", "cyclonedx-release-sbom: implemented", "signed-artifacts", "clean-install-upgrade-restore", "legacy-migration-disposition"]) if (!manifest.includes(value)) failures.push(`manifest release truth missing ${value}`);
for (const value of ["BLOCKED ON EXTERNAL EVIDENCE", "Developer ID identity", "signed update discovery", "clean-device", "legacy Wails runtime", "release runbook"]) if (!readiness.includes(value)) failures.push(`release readiness document missing ${value}`);
